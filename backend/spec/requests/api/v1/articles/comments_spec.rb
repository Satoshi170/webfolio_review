...
RSpec.describe "Api::V1::Articles::Comments", type: :request do
    ...
    context "headerが適切な場合" do
      context "有効なパラメータが指定された場合" do
        it "commnetの更新に成功する" do
          patch "/api/v1/articles/#{comment.article_id}/comments/#{comment.id}",
          params: { comment: valid_comment_params },
          headers: auth_user1_headers
  ...
  describe "DELETE /comments/:id" do
    let!(:comment) { create(:comment, user: user1, article: article) }
    context "headerが適切な場合" do
      context "有効なパラメータが指定された場合" do
        it "commnetの削除に成功する" do
          expect do
            delete "/api/v1/articles/#{comment.article_id}/comments/#{comment.id}",
            headers: auth_user1_headers
          end.to change(Comment, :count).by(-1)
